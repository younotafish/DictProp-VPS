...
while :; do
  ...
  ASSET_COUNT="$($GH_BIN release view "$RELEASE_TAG" \
    --repo "$REPO" \
    --json assets \
    --jq '[.assets[] | select(.name == "sentence-backfill.enc")] | length' \
    2>/dev/null || true)"
  if [ "$ASSET_COUNT" != "1" ]; then
    log "GitHub recovered; uploading verified archive"
  ...
  if [ ! -e "$STATE_DIR/import-triggered" ]; then
    log "required deployment succeeded; dispatching sentence backfill import"
  ...
  log "import succeeded and production is healthy"
