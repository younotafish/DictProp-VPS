...
if [[ "$image_model" != "krea2" && "$image_model" != "ernie-image-turbo" && "$image_model" != "qwen-image" && "$image_model" != "z-image-turbo" ]]; then
  echo "IMAGE_MODEL must be krea2, ernie-image-turbo, qwen-image, or z-image-turbo" >&2
  exit 2
fi
