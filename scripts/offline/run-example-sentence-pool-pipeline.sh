...
# ERNIE Turbo's four-step output matched or beat eight-step semantic QA locally.
EXAMPLE_IMAGE_STEPS="${EXAMPLE_IMAGE_STEPS:-4}"
EXAMPLE_IMAGE_SHARD_COUNT="${EXAMPLE_IMAGE_SHARD_COUNT:-1}"
...
retry env CODEX_CONCURRENCY="$IMAGE_QA_CONCURRENCY" \
  ...
  KREA_SHARD_COUNT="$EXAMPLE_IMAGE_SHARD_COUNT" bash scripts/offline/run-streaming-image-quality-loop.sh \
