...
test.describe('Study Mode Keyboard Shortcuts', () => {
  test('Space flips flashcard', async ({ seededApp: page }) => {
    await page.getByRole('button', { name: /study/i }).click();
    ...
    // Should show back of card (rating buttons visible)
    const gotItButton = page.locator('button[class*="bg-emerald"]');
    await expect(gotItButton).toBeVisible();
  });

  test('Arrow Right rates correct', async ({ seededApp: page }) => {
    await page.getByRole('button', { name: /study/i }).click();
    await page.getByRole('button', { name: /Start Session/i }).click();
  ...
  test('Arrow Left rates incorrect', async ({ seededApp: page }) => {
    await page.getByRole('button', { name: /study/i }).click();
    await page.getByRole('button', { name: /Start Session/i }).click();
    
    // Arrow left to rate as forgot
    await page.keyboard.press('ArrowLeft');
    await page.waitForTimeout(500);
    
    // Should still be in session (re-queued)
    await expect(page.getByText(/Card \d+\/\d+/)).toBeVisible();
  });

  test('1 key rates as forgot', async ({ seededApp: page }) => {
    await page.getByRole('button', { name: /study/i }).click();
    await page.getByRole('button', { name: /Start Session/i }).click();
  ...
  test('3 key rates as correct', async ({ seededApp: page }) => {
    await page.getByRole('button', { name: /study/i }).click();
    await page.getByRole('button', { name: /Start Session/i }).click();
  ...
  test('2 key triggers archive', async ({ seededApp: page }) => {
    await page.getByRole('button', { name: /study/i }).click();
    await page.getByRole('button', { name: /Start Session/i }).click();
  ...
  test('P key pronounces in study mode', async ({ seededApp: page }) => {
    await page.getByRole('button', { name: /study/i }).click();
    await page.getByRole('button', { name: /Start Session/i }).click();
...
test.describe('Touch Targets', () => {
  ...
  test('rating buttons in study mode are large enough', async ({ seededApp: page }) => {
    ...
    const forgotBox = await forgotButton.boundingBox();
    const gotItBox = await gotItButton.boundingBox();
    
    if (forgotBox && gotItBox) {
      expect(forgotBox.height).toBeGreaterThanOrEqual(40);
      expect(gotItBox.height).toBeGreaterThanOrEqual(40);
    }
  });
});

test.describe('Keyboard Only Navigation', () => {
  test('can complete full study session with keyboard only', async ({ page }) => {
    await clearIndexedDB(page);
    await clearLocalStorage(page);
    await mockFirebaseFunctions(page);
    await page.goto('/');
    ...
    // Seed with one item
    const testItems = [createStoredItem(mockVocabCard, 'vocab', { memoryStrength: 10 })];
    await seedIndexedDB(page, testItems);
    await page.reload();
    await waitForAppLoad(page);
    
    // Navigate to study with keyboard
    await page.keyboard.press('2'); // Go to study
    await expect(page.getByText("Today's Study")).toBeVisible();
    
    // Start session (Tab to button, Enter to activate)
    const startButton = page.getByRole('button', { name: /Start Session|Practice/i });
    await startButton.focus();
    await page.keyboard.press('Enter');
    
    await expect(page.getByText(/Card \d+\/\d+/)).toBeVisible();
    
    // Review card with keyboard
    await page.keyboard.press('Space'); // Flip
    await page.waitForTimeout(500);
    
    await page.keyboard.press('ArrowRight'); // Rate correct
    
    // Should complete
    await expect(page.getByText(/Brilliant/i)).toBeVisible({ timeout: 5000 });
