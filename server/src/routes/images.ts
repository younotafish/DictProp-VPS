...
import { generateImage, ImageGenerationError, type ImageAspectRatio } from '../image-generation.js';
...
imageRoutes.post('/generate-image', async (c) => {
  ...
  try {
    ...
    try {
      image = await generateImage(prompt.trim(), aspectRatio as ImageAspectRatio);
    } catch (error) {
