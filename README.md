...
- GitHub Actions runs `.github/workflows/incremental-enrichment.yml` at minute 23 every six hours. The production concurrency group prevents overlap with deploys and imports. Each run gives newly saved items a complete metadata-and-media pass, then drains the historical backlog of missing sentence explanations and generatable images until the queue is empty or its 70-minute deadline is reached; failed items do not block later batches.
